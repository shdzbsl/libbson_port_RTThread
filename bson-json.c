...
#define STACK_MAX 10
#define BSON_JSON_DEFAULT_BUF_SIZE (1024)
                     ...
                     ssize_t len)
{
   ...
   if (err != JSONSL_ERROR_SUCCESS) {
       BSON_PRINTF("BSON error: error near position %d\n", (int) state->pos_begin);
      return false;
   }
                 ...
                 char *errat)
{
   ...
   BSON_PRINTF("BSON error: Got parse error at \"%c\", position %d: \n", *errat, (int) json->pos);
   return 0;
}
                       ...
                       bson_error_t *error)        /* OUT */
{
   ...
   for (;;) {
      ...
      if (r < 0) {
         if (error) {
             BSON_PRINTF("BSON error: reader cb failed\n");
         }
         ret = -1;
         goto cleanup;
      } else if (r == 0) {
                    ...
                    bson_error_t *error) /* OUT */
{
   ...
   if (r == 0) {
       BSON_PRINTF("BSON error: Empty JSON string\n");
   }
